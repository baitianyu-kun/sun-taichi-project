...
if __name__ == '__main__':
    ...
    old_data_path = 'gesture_value.txt'
    new_data_path = 'many_gesture_value_svc_ap_100iter.txt'
    old_data = np.loadtxt('gesture_value.txt', delimiter=',')
    ...
    for old_data_index in range(len(old_data_degrees)):
        ...
        for iter in range(max_iter):
            ...
            noise_plus = np.random.randint(low=0, high=90, size=11)
            # [ -4 -12 -12  -2  -9 -20  -3  -3 -10  -5 -12]
            noise_miner = np.random.randint(low=-90, high=0, size=11)
