...
gesture = {int(x.strip().split('|')[0]): ast.literal_eval(x.strip().split('|')[1]) for x in gesture_file}
...
while (1):
    ...
    try:
        ...
        for gesture_key, gesture_value in gesture.items():
            results = [True if abs(detect_degrees[i] - gesture_value[i]) <= 30 else False for i in
                       range(len(detect_degrees))]
            print(results, Counter(results)[True])
            if Counter(results)[True] >= 10:
                print(gesture_list[gesture_key])
                break
    except:
        continue
